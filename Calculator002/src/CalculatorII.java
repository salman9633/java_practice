...
public class CalculatorII implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()== sevenButton) {
			...
			float preValvf=Float.parseFloat(prevValv);
			
			if (isaddbuttonclicked && isequalbuttonclicked) {
				isaddbuttonclicked=true;
				Float result=oldValF+newValF;
				displayLabel.setText(result+"");
				/*if (isaddbuttonclicked || issubbuttonclicked && isequalbuttonclicked) {
					isaddbuttonclicked=false;
					float prevValvf=0;
					
					float resultf = prevValvf+oldValF+newValF;
